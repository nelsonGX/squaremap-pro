...
/**
 * Mod-owned bounded worker pool for feature storage (SQLite I/O) and routing.
 *
 * <p>Daemon threads named {@code squaremap-pro-worker-N}; {@code max(1, cores / 2)} threads capped at 4;
...
public final class MapExecutor implements Executor {
   ...
   */
  public MapExecutor(int threads, int queueCapacity) {
    AtomicInteger counter = new AtomicInteger(1);
    ThreadFactory factory = r -> {
      Thread t = new Thread(r, "squaremap-pro-worker-" + counter.getAndIncrement());
      t.setDaemon(true);
      return t;
    };
