...
// THREADING: create() runs once from NavFabricMod.onInitialize (loader thread).
public final class NavMapLayers {

  private static final Logger LOGGER = LoggerFactory.getLogger("squaremap-pro");

  private NavMapLayers() {
  }
   ...
   * them executes. {@code SquaremapNavMapLayer} (the only class referencing
   ...
   * squaremap is absent neither {@code SquaremapFactory} nor {@code SquaremapNavMapLayer} is ever
   ...
   * @return the layer
   */
  public static NavMapLayer create(Supplier<MinecraftServer> server) {
    if (!FabricLoader.getInstance().isModLoaded("squaremap")) {
      LOGGER.info("squaremap not installed; routes will not be drawn on a map");
      return new NoopNavMapLayer();
    }
    try {
      NavMapLayer layer = SquaremapFactory.create(server);
      LOGGER.info("squaremap detected; navigation layer enabled");
      return layer;
    } catch (RuntimeException | LinkageError e) {
      LOGGER.warn("squaremap integration unavailable; routes will not be drawn", e);
      return new NoopNavMapLayer();
    }
  }
  ...
  private static final class SquaremapFactory {
    static NavMapLayer create(Supplier<MinecraftServer> server) {
      return new SquaremapNavMapLayer(server);
    }
  }
}
